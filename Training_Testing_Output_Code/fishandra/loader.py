...
def load_dataset(data, future_steps = 5, num_neighbours = 15,
    ...
    if cons.REMOVE_OUTER is not None:
        ...
        print(np.min(distance_to_center))
        print(np.max(distance_to_center))
        print('that is the distance hier')

        all_data["invalid"] = distance_to_center > cons.REMOVE_OUTER
        #all_data["invalid"] = np.logical_or(distance_to_center > 0.9, distance_to_center < 0.7)
